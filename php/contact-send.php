...
if( isset($_POST['name']) && empty($_POST['spam-check']) ) {
	...
	if( strlen($input_name) < 2 ) $error['name'] = 'Παρακαλώ συμπληρώστε το όνομά σας.';
	if( strlen($input_message) < 5 ) $error['message'] = 'Παρακαλώ αφήστε το μήνυμά σας.';

	// Make sure the email is valid
	if( !filter_var($input_email, FILTER_VALIDATE_EMAIL) ) $error['email'] = 'Παρακαλώ συμπληρώστε με κάποιο έγκυρο λογαριασμό email.';
	...
	if( !$error ) {
		...
		if( mail($your_email_address, $subject, $message, "From: $input_email") ) {
			echo '<p class="success">Το μήνυμά σας εστάλη! Ευχαριστούμε. </p>';
		} else {
			echo '<p class="error">Υπήρξε κάποιο πρόβλημα με την αποστολή! Παρακαλώ δοκιμάστε αργότερα!</p>';
		}
